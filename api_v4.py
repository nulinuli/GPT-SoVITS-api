...
async def tts_x(request: TTSX_Request):
    ...
    try:
        output = list(open1Ba(req["batch_size"],req["total_epochs"],req["text_low_lr_rate"],req["if_save_latest"],req["if_save_every_weights"],req["save_every_epochs"]))
    except Exception as e:
        return JSONResponse(status_code=400, content={"message": "sovits训练失败", "Exception": str(e)})
    try:
        output = list(open1Bb(req["batch_size"],req["total_epochg"],req["if_dpo"],req["if_save_latest"],req["if_save_every_weights"],req["save_every_epochg"]))
    except Exception as e:
