...
class TTSX_Request(BaseModel):
    ...
    save_every_epochs: int=12
    total_epochg: int=15
    if_dpo: bool=False
    save_every_epochg: int=15
...
def download_and_extract_zip(url):
    ...
    try:
        ...
        # 如果只提取出了一个文件夹，则返回这个文件夹的路径
        if len(extracted_items) == 1:
            folder_path = os.path.join(temp_dir, extracted_items[0])
...
def get_weights_names():
    ...
    for path in SoVITS_weight_root:
        for name in os.listdir(path):
            if name.endswith(".pth"): SoVITS_names.append("%s" %name)
    SoVITS_names = [name.removesuffix('.pth') for name in SoVITS_names]
    return SoVITS_names
...
def open_slice(inp, threshold, min_length, min_interval, hop_size, max_sil_kept, _max, alpha, n_parts):
    ...
    opt_root = "GPT-SoVITS/output/slicer%s" % cha
...
def open_asr(asr_model, asr_model_size, asr_lang, asr_precision):
    ...
    global cha
    if(p_asr==None):
        asr_inp_dir = "GPT-SoVITS/output/slicer%s" % cha
        asr_opt_dir = "GPT-SoVITS/output/asr%s" % cha
...
def open1abc(exp_name):
    global ps1abc
    global cha
    inp_text = "GPT-SoVITS/output/asr%s/slicer%s.list" % (cha,cha)
    inp_wav_dir = "GPT-SoVITS/output/slicer%s" % cha
...
def check_params(req:dict):
    ...
    ref_audio_url:str = req.get("ref_audio_url", "")
    weights_name:str = req.get("weights_name","")
    streaming_mode:bool = req.get("streaming_mode", False)
    media_type:str = req.get("media_type", "wav")
    prompt_lang:str = req.get("prompt_lang", "")
    text_split_method:str = req.get("text_split_method", "cut5")

    if ref_audio_url in [None, ""]:
        return JSONResponse(status_code=400, content={"message": "ref_audio_path is required"})
    if text in [None, ""]:
        return JSONResponse(status_code=400, content={"message": "text is required"})
    if (weights_name in [None, ""]) :
        return JSONResponse(status_code=400, content={"message": "weights_name is required"})
...
async def tts_handle(req:dict):
    ...
    try:
        ...
        # 删除临时文件
        temp_file_path = req["ref_audio_url"]
...
@APP.get("/ttsq")
async def tts_q():
    weight_names = get_weights_names()
    return JSONResponse(status_code=200, content={"weight_names": weight_names})

@APP.post("/tts")
async def tts_post_endpoint(request: TTS_Request):
    req = request.dict()
    req["ref_audio_url"] = download_and_process_file(request.ref_audio_url)
    print(req["ref_audio_url"])
    weights_name = req["weights_name"]
    try:
        if weights_name in ["", None]:
            return JSONResponse(status_code=400, content={"message": "sovits weight name is required"})
        elif os.path.exists(f"SoVITS_weights_v2/{weights_name}.pth"):
            tts_pipeline.init_vits_weights("SoVITS_weights_v2/"+weights_name+".pth")
        else:
            return JSONResponse(status_code=400, content={"message": "模型名字不存在"})
    except Exception as e:
        return JSONResponse(status_code=400, content={"message": f"change sovits weight failed", "Exception": str(e)})
    try:
        if weights_name in ["", None]:
            return JSONResponse(status_code=400, content={"message": "gpt weight name is required"})
        elif os.path.exists(f"GPT_weights_v2/{weights_name}.ckpt"):
            tts_pipeline.init_t2s_weights("GPT_weights_v2/"+weights_name+".ckpt")
        else:
            return JSONResponse(status_code=400, content={"message": "模型名字不存在"})
    except Exception as e:
...
'''
@APP.post("/ttsx")
async def tts_x(request: TTSX_Request):
    req = request.dict()
    folder_path = download_and_extract_zip(req["url"])
    if folder_path:
        print(f"The extracted folder is located at: {folder_path}")
    else:
        return JSONResponse(status_code=400, content={"message": "The contents are not a single folder."})
    try:
        output = list(open_slice(folder_path, req['threshold'], req['min_length'], req['min_interval'], req['hop_size'], req['max_sil_kept'], req['max_'], req['alpha'], req['n_parts']))
    except Exception as e:
        return JSONResponse(status_code=400, content={"message": "切分失败", "Exception": str(e)})
    if os.path.exists(folder_path):
        shutil.rmtree(folder_path)
    try:
        output = list(open_asr(req["asr_model"], req["asr_model_size"], req["asr_lang"], req["asr_precision"]))
    except Exception as e:
        return JSONResponse(status_code=400, content={"message": "asr失败", "Exception": str(e)})
    global exp_name
    exp_name = req.get('exp_name')

    if not exp_name:
        raise HTTPException(status_code=400, detail="缺少模型名字")
    try:
        output = list(open1abc(exp_name))
    except Exception as e:
        return JSONResponse(status_code=400, content={"message": "数据格式化失败", "Exception": str(e)})
    try:
        output = list(open1Ba(req["batch_size"],req["total_epochs"],req["text_low_lr_rate"],req["if_save_latest"],req["if_save_every_weights"],req["total_epochs"]))
    except Exception as e:
        return JSONResponse(status_code=400, content={"message": "sovits训练失败", "Exception": str(e)})
    try:
        output = list(open1Bb(req["batch_size"],req["total_epochg"],req["if_dpo"],req["if_save_latest"],req["if_save_every_weights"],req["total_epochg"]))
    except Exception as e:
        return JSONResponse(status_code=400, content={"message": "gpt训练失败", "Exception": str(e)})
    return JSONResponse(status_code=200, content={"message": "训练成功"})
